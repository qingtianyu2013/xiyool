...
+ (instancetype)showAlertViewWithStyle:(WKAlertViewStyle)style title:(NSString *)title detail:(NSString *)detail canleButtonTitle:(NSString *)canle okButtonTitle:(NSString *)ok callBlock:(callBack)callBack
{
    switch (style) {
        ...
        case WKAlertViewStyleWaring:
            ...
            break;
        case WKAlertViewStyleLoading:
            [[self shared] drawLoadingSize:10];
            break;
        default:
            break;
    }
...
-(void)drawLoadingSize:(int)size
{
    [self logoInit];
    CGPoint point=CGPointMake(_logoView.center.x/2, 60);
    
    view1=[[UIView alloc]initWithFrame:CGRectMake(point.x, point.y, size, size)];
    view3=[[UIView alloc]initWithFrame:CGRectMake(point.x+size*3, point.y, size, size)];
    view2=[[UIView alloc]initWithFrame:CGRectMake(point.x-size*3, point.y, size, size)];
    UIBezierPath * path = [UIBezierPath bezierPath];
    path.lineCapStyle = kCGLineCapRound;
    path.lineJoinStyle = kCGLineCapRound;
    [path addArcWithCenter:CGPointMake(size/2, size/2) radius:size startAngle:0 endAngle:180.0 clockwise:YES];
    CAShapeLayer *layer = [[CAShapeLayer alloc] init];
    ...
    layer2.strokeColor = [UIColor orangeColor].CGColor;
    layer2.lineWidth = 1;
    ...
    [_logoView addSubview:view1];
    [_logoView addSubview:view2];
    [_logoView addSubview:view3];
    thread =[[NSThread alloc]initWithTarget:self selector:@selector(changealpha) object:nil];
    [thread start];
}
-(void)changealpha
{
    while (true) {
        ...
        dispatch_sync(dispatch_get_main_queue(),^{
            ...
            if (view1.alpha>=0.6 && view1.alpha<0.9) {
                view1.alpha=0.3;
                view2.alpha=0.9;
                view3.alpha=0.6;
            }else if(view1.alpha>=0.3 && view1.alpha<0.6)
            {
...
-(void)ChangeStyleWKAlertViewStyle:(WKAlertViewStyle)style
{
    thread=nil;
    view1.hidden=YES;
    view1=nil;
